...
def analyze_per_img_dets(confusion_matrix,
    ...
    for det_label in unique_label:
        ...
        # print(f"Stripe predictions: {det_bboxes[det_label == 'stripe']}")
        # print(f"Stripe GTs: {gt_bboxes[gt_labels == 'stripe']}")
...
def calculate_and_print_metrics(confusion_matrix, labels):
    num_classes = len(labels)
    tp = np.diag(confusion_matrix)[:num_classes]
    fp = confusion_matrix[:-1, :-1].sum(axis=0) - tp
    fn = confusion_matrix[:-1, :-1].sum(axis=1) - tp
    tn = confusion_matrix[:-1, :-1].sum() - (tp + fp + fn)

    accuracy = (tp + tn) / confusion_matrix[:-1, :-1].sum()
...
def main():
    ...
    # 성능 지표 계산 및 출력
    calculate_and_print_metrics(confusion_matrix, dataset.metainfo['classes'])
